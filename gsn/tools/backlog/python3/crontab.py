...
__license__     = "GPL"
__version__     = "$Revision: 1929 $"
__date__        = "$Date: 2010-06-01 17:25:59 +0200 (Die, 01. Jun 2010) $"
__id__          = "$Id: crontab.py 1929 2010-06-01 15:25:59Z tgsell $"
__source__      = "$URL: https://gsn.svn.sourceforge.net/svnroot/gsn/branches/permasense/gsn/tools/backlog/python/crontab.py $"
...
class CronItem(object):
    ...
    def __init__(self, line=None, command='', meta=''):
        self.command = CronCommand(str(command))
        self._meta   = meta
        self.valid   = False
        self.slices  = []
        self.special = False
    ...
    def render(self):
        ...
        if not self.special:
            slices = []
            for i in range(0, 5):
                slices.append(str(self.slices[i]))
            time = ' '.join(slices)
        if self.special or time in list(SPECIALS.values()):
            if self.special:
                time = self.special
            else:
                time = "@%s" % list(SPECIALS.keys())[list(SPECIALS.values()).index(time)]

        result = "%s %s" % (time, str(self.command))
        if self.meta():
            result += " # " + self.meta()
        return result
