...
class AbstractPluginClass(Thread, Statistics):
    ...
    def registerDPPListener(self, types, excempted=False):
        '''
        Register a plugin as a DPP listener.
        
        This function should be used by the plugins to register themselves as DPP listeners.
        After registering the specified DPP message types will be received with dppMsgReceived(...).
        
        @param types: A list containing all DPP message types this listener is listening to.
        
        @param excempted: if set to True all DPP message types will be listened to except the ones
                          specified in types.
        
        @raise Exception: if the DPPPeerClass can not be started.
        '''
        self._backlogMain.registerDPPListener(self, types, excempted)
    
    
    def deregisterDPPListener(self):
        '''
        Deregister a plugin from DPP peer.
        
        This function should be used by the plugins to deregister themselves from the DPP peer.
        After deregistering no more DPP messages will be received with dppMsgReceived(...).
        If a plugin registered itself with registerDPPListener(), this function has to be called
        at least once, if stop() is called!
        '''
        self._backlogMain.deregisterDPPListener(self)
    
    
    def dppMsgReceived(self, timestamp, dppMsg):
        '''
        This function will be executed if a DPP message has been received from the I2C
        port and this plugin listening to the specific DPP message type.
        
        @param dppMsg: the DPP message as specified in DPPTypes
    ...
    def sendDPPmsg(self, dppMsg):
        '''
        Send a DPP message over the I2C port.
        ...
        '''
        return self._backlogMain._dpppeer.sendDPPMsg(dppMsg)
