...
public class EMMobileDataParserWIFI extends BridgeVirtualSensorPermasense {

	private static final transient Logger logger = Logger.getLogger(EMMobileDataParserWIFI.class);
	...
	private static DataField[] dataField = {
			...
			new DataField("SSID", "VARCHAR(256)"),
			new DataField("CAPABILITY", "VARCHAR(256)"),
			new DataField("FREQ", "INTEGER"),
  ...
  private double getFieldStrength(int rssi, long freq, double gain) {
	...
	private void parseData(File file, String inputStreamName, StreamElement data) {
        ...
        double fs = getFieldStrength(readingsInt[1], 2400000000L, 1.0);
				
				StreamElement curr_data = new StreamElement(dataField, new Serializable[] {data.getData(dataField[1].getName()), data.getData(dataField[1].getName()), timestamp, tokens[3], tokens[4], readingsInt[0], readingsInt[1], fs, readingsDouble[0], readingsDouble[1], null});
