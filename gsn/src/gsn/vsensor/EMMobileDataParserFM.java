...
public class EMMobileDataParserFM extends BridgeVirtualSensorPermasense {

	private static final transient Logger logger = Logger.getLogger(EMMobileDataParserFM.class);
	...
	private void parseData(File file, String inputStreamName, StreamElement data) {
        ...
        if (timestamp == null || timestamp == 0 || readingsInt[1] == null ||
          ...
          continue;
				...
				double fs = getFieldStrength(readingsInt[1], 100000000L, 1.0);
				
				StreamElement curr_data = new StreamElement(dataField, new Serializable[] {data.getData(dataField[1].getName()), data.getData(dataField[1].getName()), timestamp, readingsInt[0], readingsInt[1], fs, readingsDouble[0], readingsDouble[1], null});
	...
	public void dataAvailable(String inputStreamName, StreamElement data) {
		...
		if (file.getAbsolutePath().indexOf("fm") == -1)
		  return;
