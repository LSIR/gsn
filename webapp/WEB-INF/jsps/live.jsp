<%@page import="java.util.*,gsn.beans.*,gsn.storage.*,gsn.web.*,java.sql.*" %>
<% 
   Object data = request.getAttribute ( AllDataFromVirtualSensor.KEY );
   if ( data == null )
      return;
   ResultSet result = ( ResultSet ) data;
   String vsName = ( String ) request.getAttribute ( AllDataFromVirtualSensor.VS_NAME );
   ArrayList<DataField> dataFields =(ArrayList<DataField> ) request.getAttribute ( AllDataFromVirtualSensor.OUTPUT_STRUCTURE );
    Integer pageSize = ( Integer ) request.getAttribute ( AllDataFromVirtualSensor.NUM_OF_ITEMS_KEY );
//   Integer    pageStartIndex = ( Integer ) request.getAttribute ( AllDataFromVirtualSensor.OUTPUT_START_INDEX );
    Integer refresh_rate = (Integer) request.getAttribute(AllDataFromVirtualSensor.REFRESH_RATE_KEY);
      ...
      <center>
		 ...
		 <table width="75%" cellpadding=0 cellspacing=0 border=0>
			  <tr>
               ...
               <td> <input type="text" name="<%=AllDataFromVirtualSensor.REFRESH_RATE_KEY%>" value=<%=refresh_rate%> /> </td>
	 	      </tr>
        	       <td> Number of items : </td>
               <td> <input type="text" name="<%=AllDataFromVirtualSensor.NUM_OF_ITEMS_KEY%>"  value=<%=pageSize%> /> </td>
 			  </tr><tr>
         ...
         <table width="75%" cellpadding=0 cellspacing=0 border=0>
            ...
            <tr>
               ...
               <td><center>
                  ...
                  <IMG SRC="field.do?vs=<%=vsName%>&identity=<%=primaryKey%>&field=<%=result.getMetaData ().getColumnName ( i )%>">
                  <%} else if (mimeType.equalsIgnoreCase ( "svg")) {%>
                   <EMBED type="image/svg+xml" width="400" height="400" src="field.do?vs=<%=vsName%>&identity=<%=primaryKey%>&field=<%=result.getMetaData ().getColumnName ( i )%>&type=svg"   PLUGINSPAGE="http://www.adobe.com/svg/viewer/install/" />
                  <%} %>
               </center></td>
               <%} else if ( result.getMetaData ().getColumnName ( i ).equals ( "TIMED" ) ) {%>
               <td><center>
                  <%=new Time ( Long.parseLong ( result.getString ( i ) ) )%>
               </center></td>
