...
FilterAltitude.prototype.initialize = function(map, markers, data){
    ...
    $( "#minAlt" ).keyup(function() {
        ...
        if (newMin <= maxAlt && newMin >= minAlt){
            ...
            for (var i = 0; i < Object.keys(data[category]).length; i++) {
                if (typeof data[category][i]['Coordinates'] == 'undefined') continue;
                if (typeof data[category][i]['Altitude'] == 'undefined' || isNaN(data[category][i]['Altitude'])) {
                    markersCond[i][myNumOfFilter] = false;
                    stateChanged(i);
                    continue;
                }
                if ((parseFloat(data[category][i]['Altitude']) >= $("#minAlt").val())
                    && (parseFloat(data[category][i]['Altitude']) <= $("#maxAlt").val())) {
                    markersCond[i][myNumOfFilter] = true;
                } else {
                    markersCond[i][myNumOfFilter] = false;
                }
    ...
    $("#slider-range").slider({
        ...
        stop: function (event, ui) {
            ...
            for (var i = 0; i < Object.keys(data[category]).length; i++) {
                ...
                if ((parseFloat(data[category][i]['Altitude']) >= parseFloat($("#minAlt").val()))
                    && (parseFloat(data[category][i]['Altitude']) <= parseFloat($("#maxAlt").val()))) {
                    markersCond[i][myNumOfFilter] = true;
                } else {
                    markersCond[i][myNumOfFilter] = false;
                }
