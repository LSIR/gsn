...
public class DummyRemoteDataProducer extends AbstractStreamProducer {
    ...
    private static final transient Logger logger = Logger
	    .getLogger(DummyDataListener.class);
	...
	if (port <= 0) {
	    logger.error("The port " + port
		    + " is invalid for a stream source.");
	    return false;
	}
	remoteVSName = (String) initialContext.get(Registry.VS_NAME);
	if (remoteVSName == null) {
	    logger
		    .warn("The \"NAME\" parameter of the AddressBean which corresponds to the remote Virtual Sensor is missing");
	    return false;
	}
	if (logger.isDebugEnabled())
	    logger.debug(new StringBuilder().append(
		    "The DummyRemoteDataProducer is initialized with :[host=")
		    .append(host).append(":").append(port).append(", VSName=")
	...
	postMethod.addRequestHeader(Registry.VS_PORT, Integer
		.toString(containerPort));
	ArrayList<String> tables = new ArrayList<String>();
	tables.add(remoteVSName);
	StringBuffer generateQuery = QueryGenerator.generateQuery("\""
		+ remoteVSName + ".DATA" + "\"", tables, 2, tables.size(),
		10000000);

	postMethod.addRequestHeader(Container.VS_QUERY, generateQuery
		.toString());
