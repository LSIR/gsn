...
public class Simulation {
    ...
    public Simulation(VSensorConfig configuration, int numOfThreads)
	    throws Exception {
	Server servers[] = new Server[numOfThreads];
	TreeMap<String, Object> context1 = new TreeMap<String, Object>(
	...
	for (int i = 0; i < numOfThreads; i++) {
		...
		logger.info(new StringBuilder().append("The port ").append(
			portCounter).append(" used successfully.").toString());
	    if (logger.isDebugEnabled())
		logger.debug(new StringBuilder().append(
			"The Http Server of the ").append(i).append(
			"th container initialized").toString());
		...
		for (StreamSource streamSource : inputStream.getSources()) {
			...
			if (dummyRemoteDataSource == null) {
				...
				context1.put(VSensorLoader.STREAM_SOURCE,
					streamSource);
				context1.put(VSensorLoader.INPUT_STREAM,
					inputStream);
				context1.put(Registry.VS_HOST, addressBean
					.getPredicateValue(Registry.VS_HOST));
				context1.put(Registry.VS_PORT, addressBean
					.getPredicateValue(Registry.VS_PORT));
				context1.put(Registry.VS_NAME, addressBean
					.getPredicateValue(Registry.VS_NAME));
				context1.put(DummyDataListener.CONTAINER_PORT,
					(START_PORT_INDEX + i));
				boolean output = dummyRemoteDataSource
					.initialize(context1);
				if (!output) {
				    logger
					    .error("dummyRemoteDataSource's initialization failed.");
				    System.exit(1);
				}
			    }
			}
	    ...
	    if (logger.isDebugEnabled())
		logger.debug(new StringBuilder().append(
			"The input stream of the ").append(i).append(
			"th container initialized").toString());
    ...
    /**
         * The args[0]; the first parameter; is the configuration file address.
         * The args[1]; the second parameter; is the log4J param. The args[2];
         * the third parameter; is the number of dummy clients to be simulated.
         * The args[3]; the forth parameter; is the start index of the port (the
         * program will use portStart+1 to portIndex+args[2])
         * 
         * @throws Exception
         */
	...
	VSensorConfig configuration = (VSensorConfig) uctx.unmarshalDocument(
