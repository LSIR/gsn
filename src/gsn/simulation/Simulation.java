...
public class Simulation {
   ...
   public Simulation ( VSensorConfig configuration , int numOfThreads ) throws Exception {
      ...
      for ( int i = 0 ; i < numOfThreads ; i ++ ) {
         ...
         // servers [ i ].addListener ( ":" + ( START_PORT_INDEX + ++portCounter
         // ) ) ;
         ...
         // // logger.warn ( "The Port :" + ( portounter ) + " is used, the port
         // is skipped" ) ;
         // i-- ;
         // continue ;
         // }
         if ( logger.isInfoEnabled ( ) )
            logger.info ( new StringBuilder ( ).append ( "The port " ).append ( portCounter ).append ( " used successfully." ).toString ( ) ) ;
         if ( logger.isDebugEnabled ( ) )
            logger.debug ( new StringBuilder ( ).append ( "The Http Server of the " ).append ( i ).append ( "th container initialized" ).toString ( ) ) ;
         ...
         if ( logger.isDebugEnabled ( ) )
            logger.debug ( new StringBuilder ( ).append ( "The input stream of the " ).append ( i ).append ( "th container initialized" ).toString ( ) ) ;
