...
public class DataListener {
    ...
    /**
         * The name is the very name of the Table name in the storage manager
         * which is used for storing data; (e.g., DataBaseAliasName).
         */
	...
	this.inputStream.addToRenamingMapping(streamSource.getAlias(),
		getViewName());
	this.storageManager = (StorageManager) map
	...
	if (logger.isDebugEnabled())
	    logger.debug(new StringBuilder().append(
		    "Data avialble in the stream *").append(
		    streamSource.getAlias()).append("*").toString());
	...
	if (viewQery == null)
	    viewQery = new StringBuilder("select * from "
		    + getViewName().replace("\"", ""));
	return viewQery;
    }
	...
	boolean needsWhere = (toReturn.toString().toUpperCase().indexOf(
		" WHERE ") <= 0);

	if (needsWhere) // Applying the ** End Time **
	    toReturn.append(" where ").append(" (\"wrapper.TIMED\" <=").append(
		    streamSrc.getEndDate().getTime()).append(")");
	else {
	    toReturn.append(" AND (\"wrapper.TIMED\" <=").append(
		    streamSrc.getEndDate().getTime()).append(")");
	}
	// Applying the ** START TIME **
	toReturn.append(" AND (\"wrapper.TIMED\" >=").append(
		streamSrc.getStartDate().getTime()).append(")");
	...
	if (rate == 0)
	    logger
		    .warn(new StringBuilder()
			    .append(
				    "The sampling rate is set to zero which means no results. (InputStream = ")
			    .append(dataListener.getInputStream()).append(
				    ", StreamSource = ").append(
				    streamSrc.getAlias()).toString());
	toReturn.append(" AND ( mod( abs(").append(System.currentTimeMillis())
		.append(" - \"wrapper.TIMED\")*11  , 100)< ").append(rate)
		.append(")");
	...
	if (historySize != null)
		...
		toReturn.append(" order by \"wrapper.TIMED\" desc  limit  ")
			.append(value).append(" offset 0");
	    } else { // Applying the ** Timing based History Size **
		if (StorageManager.isHsql()) {
		    if (historySize.toLowerCase().trim().endsWith("m")) {
			int miniute = Integer.parseInt(historySize
				.toLowerCase().replace("m", ""));
			toReturn.append(
				" AND ((NOW_MILLIS() - \"wrapper.TIMED\") <=")
				.append(1000 * 60 * miniute).append(" )");
		    } else if (historySize.toLowerCase().trim().endsWith("s")) {
			int seconds = Integer.parseInt(historySize
				.toLowerCase().trim().replace("s", ""));
			toReturn.append(
				" AND ((NOW_MILLIS() - \"wrapper.TIMED\") <=")
				.append(1000 * seconds).append(" )");
		    } else if (historySize.toLowerCase().trim().endsWith("h")) {
			int hours = Integer.parseInt(historySize.toLowerCase()
				.trim().replace("h", ""));
			toReturn.append(
				" AND ((NOW_MILLIS() - \"wrapper.TIMED\") <=")
				.append(1000 * 60 * 60 * hours).append(" )");
		    }
			...
			toReturn
				.append(
					" AND ((UNIX_TIMESTAMP() - \"wrapper.TIMED\") <=")
				.append(1000 * 60 * miniute).append(" )");
		    } else if (historySize.toLowerCase().trim().endsWith("s")) {
			int seconds = Integer.parseInt(historySize
				.toLowerCase().trim().replace("s", ""));
	...
	if (logger.isDebugEnabled()) {
	    logger.debug(new StringBuilder().append("The original Query : \"")
		    .append(streamSrc.getSqlQuery()).append("\"").toString());
	    logger.debug(new StringBuilder().append("The merged query : ")
		    .append(toReturn.toString()).append(
			    " of the StreamSource \"").append(
			    streamSrc.getAlias()).append(
			    "\" of the InputStream:\"").append(
			    dataListener.getInputStream().getInputStreamName())
		    .append("\"").toString());
	}
    ...
    /**
         ...
         * This method also caches it's result in the
         * <code>cachedWhereClause</code>.
         */
	...
	if (cachedWhereClause == null) {
	    ...
	    int indexOfWhereClause = mergedQuery.toUpperCase().indexOf(
		    " WHERE ");
	    cachedWhereClause = new StringBuffer(mergedQuery.substring(
		    indexOfWhereClause + " WHERE ".length(),
		    (indexOrOrderBy > 0 ? indexOrOrderBy : getMergedQuery()
			    .length())));
	    HashMap<String, String> rewritingMapping = new HashMap<String, String>();
	    rewritingMapping.put("WRAPPER", remoteVSName);
	    cachedWhereClause = new StringBuffer(SQLUtils.rewriteQuery(
		    cachedWhereClause.toString(), rewritingMapping));
	    if (logger.isDebugEnabled())
		logger
			.debug(new StringBuilder()
				.append(
					"The Complete Mereged Query's where part, rewritten for *")
				.append(remoteVSName).append("* is ").append(
					cachedWhereClause.toString())
				.toString());
	}
