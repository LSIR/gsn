...
public class UDPWrapper extends AbstractStreamProducer {
    ...
    /*
         ...
         * 
         * port : the udp port it should be listening to rate : time to sleep
         * between each packet
         */
	...
	while (isActive()) {
		...
		StreamElement streamElement = new StreamElement(
			...
			new Serializable[] { receivedPacket.getData() }, System
				.currentTimeMillis());
		publishData(streamElement);
	    } catch (IOException e) {
		logger.warn("Error while receiving data on UDP socket : "
			+ e.getMessage());
	    }
	}
    }
	...
	dataField.add(new DataField(RAW_PACKET, "BINARY",
		"The packet contains raw data received as a UDP packet."));
	return dataField;
    }
