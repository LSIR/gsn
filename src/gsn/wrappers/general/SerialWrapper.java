...
public class SerialWrapper extends AbstractStreamProducer implements SerialPortEventListener {
   ...
   public class SerialConnection {
      ...
      public void openConnection ( ) throws SerialConnectionException {
         // parameters = new SerialParameters("/dev/ttyS0", 9600, 0, 0, 8, 1,
         // 1);
   ...
   public void run ( ) {
      while ( isActive ( ) ) {
         ...
         StreamElement streamElement = new StreamElement (
               new String [ ] { RAW_PACKET } , new Integer [ ] { DataTypes.BINARY } , new Serializable [ ] { inputBuffer } , System.currentTimeMillis ( ) ) ;
   ...
   public void serialEvent ( SerialPortEvent e ) {
      ...
      if ( logger.isDebugEnabled ( ) )
         logger.debug ( "Serial wrapper received a serial port event, reading..." ) ;
      ...
      case SerialPortEvent.DATA_AVAILABLE :
          ...
          * (logger.isDebugEnabled ()) logger.debug("SerialWrapper: Warning, is ==
          * null !"); is = wnetPort.getInputStream(); } else newData =
          * is.read(); if (newData > -1 && newData < 256) { inputBuffer[index++] =
          * (byte) newData; } } catch (IOException ex) { System.err.println(ex);
          * return; } }
          */
         try {
            is.read ( inputBuffer ) ;
      ...
      if ( logger.isDebugEnabled ( ) )
         logger.debug ( "Serial port wrapper processed a serial port event, stringbuffer is now : " + new String ( inputBuffer ) ) ;
   }
