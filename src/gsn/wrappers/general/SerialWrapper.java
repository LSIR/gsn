 ...
 * 
 * Links GSN to a sensor network through serial port. <p/> The only needed
 * parameter is the serial port address, provided through xml. Default
 * connection settings are 9600 8 N 1 (I had some problems with javax.comm Linux
 * when trying to use non-default settings) TODO parametrize connection settings
 * through xml.
 * 
 * @author Ali Salehi (AliS, ali.salehi-at-epfl.ch)<br>
 * @author Jerome Rousselot CSEM<br>
 */
public class SerialWrapper extends AbstractStreamProducer implements
	SerialPortEventListener {

    private static final String RAW_PACKET = "RAW_PACKET";

    private final transient Logger logger = Logger
	    .getLogger(SerialWrapper.class);
    ...
    /*
         ...
         * OR ITS DERIVATIVES. IN NO EVENT WILL SUN OR ITS LICENSORS BE LIABLE
         * FOR ANY LOST REVENUE, PROFIT OR DATA, OR FOR DIRECT, INDIRECT,
         * SPECIAL, CONSEQUENTIAL, INCIDENTAL OR PUNITIVE DAMAGES, HOWEVER
         * CAUSED AND REGARDLESS OF THE THEORY OF LIABILITY, ARISING OUT OF THE
         * USE OF OR INABILITY TO USE SOFTWARE, EVEN IF SUN HAS BEEN ADVISED OF
	...
	/**
         * Creates a SerialConnection object and initialiazes variables passed
         * in as params.
         * 
         * @param serialPort
	...
	/**
         * Attempts to open a serial connection (9600 8N1). If it is
         * unsuccesfull at any step it returns the port to a closed state,
         * throws a <code>SerialConnectionException</code>, and returns. <p/>
         * Gives a timeout of 30 seconds on the portOpen to allow other
         * applications to reliquish the port if have it open and no longer need
         * it.
         */
	public void openConnection() throws SerialConnectionException {
		...
		System.out.println("port owned by so else");
	    try {
	...
	try {
		...
		if (logger.isDebugEnabled())
		    logger
			    .debug("Serial port wrapper successfully opened port and registered itself as listener.");

	    }

	} catch (SerialConnectionException e) {
	    System.err.println("Serial Port Connection Exception : "
		    + e.getMessage());
	    logger
	...
	while (isActive()) {
	    ...
	    StreamElement streamElement = new StreamElement(
		    ...
		    new Serializable[] { inputBuffer }, System
			    .currentTimeMillis());
	...
	dataField.add(new DataField(RAW_PACKET, "BINARY",
	...
	if (logger.isDebugEnabled())
	    logger
		    .debug("Serial wrapper received a serial port event, reading...");
	...
	case SerialPortEvent.DATA_AVAILABLE:
	    /*
                 * int index = 0; while (newData != -1) { try { if (is == null) {
                 * if (logger.isDebugEnabled ()) logger.debug("SerialWrapper:
                 * Warning, is == null !"); is = wnetPort.getInputStream(); }
                 * else newData = is.read(); if (newData > -1 && newData < 256) {
                 * inputBuffer[index++] = (byte) newData; } } catch (IOException
                 * ex) { System.err.println(ex); return; } }
                 */
	    try {
		is.read(inputBuffer);
	...
	if (logger.isDebugEnabled())
	    logger
		    .debug("Serial port wrapper processed a serial port event, stringbuffer is now : "
