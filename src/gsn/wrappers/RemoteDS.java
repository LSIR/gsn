...
public class RemoteDS extends AbstractStreamProducer {
	...
	if (this.remoteVSName == null) {
	    logger
		    .warn("The \"NAME\" paramter of the AddressBean which corresponds to the remote Virtual Sensor is missing");
	    return false;
	}
	...
	if (host.indexOf("http://") < 0)
	    host = "http://" + host;
	String destination = new StringBuilder().append(host).append(":")
		.append(getAddressBeanActivePort()).append("/gsn").toString();
	if (logger.isInfoEnabled())
	    logger.info(new StringBuilder().append(
		    "Wants to ask for structure from : ").append(destination)
		    .toString());
	PostMethod postMethod = new PostMethod(destination);
	postMethod.setRequestHeader(Container.REQUEST, Integer
		.toString(Container.DATA_STRCTURE_REQUEST));
	postMethod.setRequestHeader(Container.QUERY_VS_NAME, remoteVSName);
	if (logger.isDebugEnabled()) {
	    logger.debug(new StringBuilder("Post request contains : ").append(
		    "QUERY_VS_NAME = ").append(remoteVSName).append(";")
	...
	if (statusCode == -1) {
	    logger.warn(new StringBuilder().append(
		    "Message couldn't be sent to :").append(
		    postMethod.getHostConfiguration().getHostURL()).toString());
	    return null;
	}

	if (postMethod.getResponseHeader(Container.RES_STATUS) == null
		|| postMethod.getResponseHeader(Container.RES_STATUS)
			.getValue().equals(Container.INVALID_REQUEST)) {
	    logger.debug("The respond from server : "
		    + postMethod.getResponseHeader(Container.RES_STATUS));
	    return null;
	}
    ...
    /**
         ...
         * query and afterwards it will send a <code>register</code> request
         * for adding the new version of the query.<br>
         */
    private void refreshRemotelyRegisteredQuery() {
	String notificationCode = getDBAlias();
	String host = getAddressBeanActiveHostName();
	String query = new StringBuffer("SELECT * FROM ").append(remoteVSName)
		.append(" WHERE ").append(getWhereClausesAllTogher()).append(
			" ORDER BY ").append(remoteVSName).append(
	...
	if (logger.isDebugEnabled())
	    logger.debug(new StringBuilder().append(
		    "Wants to send message to : ").append(destination).append(
		    "  for DEREGISTERING the previous query").toString());
	PostMethod postMethod = new PostMethod(destination);
	postMethod.addRequestHeader(Container.REQUEST, Integer
		.toString(Container.DEREGISTER_PACKET));
	...
	postMethod.addRequestHeader(Container.NOTIFICATION_CODE,
		notificationCode);

	int statusCode = TCPConnPool.executeMethod(postMethod);
	if (statusCode == -1) {
	    logger.warn("Message couldn't be sent to :"
		    + postMethod.getHostConfiguration().getHostURL());
	}

	if (logger.isDebugEnabled())
	    logger
		    .debug(new StringBuilder().append(
			    "Wants to send message to : ").append(destination)
			    .append(" with the query ->").append(query).append(
				    "<-").toString());
	postMethod.setRequestHeader(Container.REQUEST, Integer
		.toString(Container.REGISTER_PACKET));
	...
	StringBuffer completeMergedWhereClause = dataListener
		.getCompleteMergedWhereClause(remoteVSName);
	...
	registeredWhereClauses.remove(dataListener
		.getCompleteMergedWhereClause(remoteVSName));
    ...
    /**
         * The container is going to notify the <code>RemoteDS</code> about
         * arrival of a new data by calling this method. (note that, container
         * will first insert the data into the appropriate database and then
	...
	for (Iterator<DataListener> iterator = listeners.iterator(); iterator
		...
		if (logger.isDebugEnabled())
		    logger.debug(new StringBuilder().append(
			    "There are listeners, notify the ").append(
	...
	return result
		.delete(result.length() - " OR ".length(), result.length())
		.toString();
    }
}
