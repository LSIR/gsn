...
public class DummyRandomDataProducer extends AbstractStreamProducer {
   ...
   /**
    * The stream element could be produced on a regular bases such as an stream
    * element each 10 minutes or it can be produced randomly (e.g., when a
    * sensor see a special rfid tag). The default value is <code>-1</code>
    * meaning the stream elements will be produced randomly with no regular
    * bases. If one wants to produces the stream on regular bases, one should
    * set the following variable reflecting the interval between two stream
    * production.
    */
   ...
   /**
    * The data part of the output stream element is an integer in the range of
    * zero to the value specified in <code>data_range</code>. The default
    * <code>data_range</code> is specified below. The code
    */
   ...
   /**
    * Specifies the interval between to subsequent stream elements produced by
    * the wrapper. Default is 500msec.
    */
   private int countOutputPeriodInMSec = 500 ;

   private final transient Logger logger = Logger.getLogger ( DummyRandomDataProducer.class ) ;
   ...
   public boolean initialize ( TreeMap context ) {
      ...
      if ( addressBean.getPredicateValue ( "size" ) != null ) {
         streamElementSize = Integer.parseInt ( ( String ) addressBean.getPredicateValue ( "size" ) ) ;
      }
      if ( addressBean.getPredicateValue ( "rate" ) != null ) {
         streamOutputRate = Integer.parseInt ( ( String ) addressBean.getPredicateValue ( "rate" ) ) ;
         rateBased = true ;
      }

      if ( addressBean.getPredicateValue ( "burst-probability" ) != null ) {
         burstProbability = Double.parseDouble ( ( String ) addressBean.getPredicateValue ( "burst-probability" ) ) ;
      }
      if ( addressBean.getPredicateValue ( "burst-max-size" ) != null ) {
         burstMaxSize = Integer.parseInt ( ( String ) addressBean.getPredicateValue ( "burst-max-size" ) ) ;
      }

      if ( addressBean.getPredicateValue ( "count" ) != null ) {
         count = Integer.parseInt ( ( String ) addressBean.getPredicateValue ( "count" ) ) ;
      }

      if ( addressBean.getPredicateValue ( "count-period" ) != null ) {
         countOutputPeriodInMSec = Integer.parseInt ( ( String ) addressBean.getPredicateValue ( "count-period" ) ) ;
      }

      if ( addressBean.getPredicateValue ( "range" ) != null ) {
         data_range = Integer.parseInt ( ( String ) addressBean.getPredicateValue ( "range" ) ) ;
      }
      if ( addressBean.getPredicateValue ( "log" ) != null ) {
         // if ( Integer.parseInt ( ( String ) addressBean.getPredicateValue (
         // "log" ) ) == 1 )
         // SimulationResult.initialize ( streamElementSize, streamOutputRate,
         // burstProbability, burstMaxSize, countOutputPeriodInMSec, count );
      }
      ...
      while ( isActive ( ) ) {
         ...
         for ( int burstCounter = 0 ; burstCounter < actualBurstCount ; burstCounter ++ ) {
            ...
            StreamElement streamElement = new StreamElement (
                  new String [ ] { "DATA" , "DATA_BIN" } , new Integer [ ] { DataTypes.INTEGER , DataTypes.BINARY } , new Serializable [ ] { value ,
                        testBinaryData } , System.currentTimeMillis ( ) ) ;
         ...
         if ( rateBased )
            ...
            } else if ( actualCount <= 0 && actualCountOutputPeriodInMSec > 0 ) {
               sleepPeriod = actualCountOutputPeriodInMSec + ( int ) ( Math.random ( ) * countOutputPeriodInMSec ) ;
