...
public class MoteIdentifier extends AbstractStreamProducer implements MessageListener, ChangeListener {
    ...
    /**
     * 1. GSN Data Containing SensorReadings
     * 2. TEDS ID Packet sent by the mote containing TEDS_ID.
     * its simple.. In this file.. we don't care about the GSNMessages.....
     * all we care about is TEDSMessages..
     * Once we get the TEDSMessages.... We create corrsponding VS xmls..
     *
    ...
    public Collection<DataField> getProducedStreamStructure() {
        if (outputStructureCache == null) {
            ...
            outputStructureCache.add(new DataField(ID_OUTPUT_FIELD, "varchar(20)", "Id of the detected transducer"));
            outputStructureCache.add(new DataField(TEDS_OUTPUT_FIELD, "VARCHAR(10000)", "TEDS-data"));
            outputStructureCache.add(new DataField(STATUS_OUTPUT_FIELD, "VARCHAR(20)", "status:added or removed"));
            outputStructureCache.add(new DataField(VSFILE_OUTPUT_FIELD, "VARCHAR(40)", "Virtual Sensor Filename"));
        }
