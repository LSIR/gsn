...
public class TedsToVirtualSensor {
    ...
    /*
         * The Virtual Sensor template file. The template files are usually of
         * the form "filename.st" Here you have to specify the just the
         * "filename" without any extension
         */
    public String templatefile;

    /*
         * The directory in which the generated Virtual Sensor file should be
         * stored.
         */
	...
	StringTemplateGroup grp = new StringTemplateGroup("myGroup",
		templateDir);
	...
	try {
	    ...
	    ch0.NumberOfChannels = Integer.parseInt(r.get(
		    MeasAttr.NUMBER_OF_CHANNELS).toString());
	    if (r.get(MeasAttr.IP) == null)
		ch0.IP = "localhost";
	    else
		...
		result = new PrintWriter(new FileWriter(TARGET_VS_DIR
			+ ch0.name + ".xml"));
	    } catch (IOException e) {
		...
		System.out
			.println("Error in generating the VirtualSensor File;"
				+ e.getMessage());
		return new TedsToVSResult(-1); // End the program.
	    } finally {
	...
	return new TedsToVSResult(ch0.name + ".xml", 0, sensorTEDS
		.toHtmlString(), ch0.name);
    }
	...
	// chan0 - meta data about the TIM itself
		{ { MeasAttr.NAME, "SampleTEDSOne" },
			{ MeasAttr.DESCRIPTION, "A Simple TEDS" },
			{ MeasAttr.VERSION, "1.0" },
			{ MeasAttr.LOCATION, "INM 035" },
			{ MeasAttr.NUMBER_OF_CHANNELS, "2" },
			{ MeasAttr.MANUFACTURER, "GSN-Mica2-Network" },
			{ MeasAttr.METADATA_ID, "Channel 0" } },
		// chan1
		{ { MeasAttr.NAME, "DATA" },
			{ MeasAttr.DESCRIPTION, "Temperature value" },
			{ MeasAttr.UNITS, "Celcius" },
			{ MeasAttr.DATA_TYPE, "integer" },
			{ MeasAttr.METADATA_ID, "Channel 1" } },
		// chan2
		{ { MeasAttr.NAME, "Light" },
			{ MeasAttr.DESCRIPTION, "Light value" },
			{ MeasAttr.UNITS, "lumens" },
			{ MeasAttr.DATA_TYPE, "double" },
			{ MeasAttr.METADATA_ID, "Channel 2" } },
	...
	if (res.status == TedsToVSResult.ERROR) {
	    System.out
		    .println("**Error** in creating TEDS-VS. Check for TedsToVirtualSensor.java File");
	} else {
	...
	String fileName = sensorTEDS.GetChannel(0).get(MeasAttr.NAME)
		.toString();
	return new TedsToVSResult(fileName + ".xml", 0, sensorTEDS
		.toHtmlString(), fileName);
    }
