...
public class TEDS {
    /**
         ...
         * getMetaData() requests while the other is a Vector to provide number
         * based indexing.
         */
    public Record metaData = new Record();

    /**
         * A list of all the current meta data for the TIM. Index zero contains
         * meta data for the TIM itself.
         */
    protected Vector metaDataVector = new Vector();

    /**
         * A helper object containing a mapping from channel name to channel
         * number.
         */
    ...
    /**
         ...
         * 
         * @param chNum -
         *                The integer channel number used for reads. Use -1 to
         *                allow automatic number incrementing and replacment of
         *                duplicate channel name teds.
         * @param chanMetaData -
	...
	if (chNum == -1 && (metaDataVector.size() == 0))
	    chNum = 0;

	if (chNum == 0) {
	    chanMetaData.put(MeasAttr.METADATA_TYPE, MeasAttr.METADATA_T_TIM);
	} else {
	    chanMetaData.put(MeasAttr.METADATA_TYPE,
		    MeasAttr.METADATA_T_CHANNEL);
	}
	chanMetaData.lock();
	...
	if (chNum == -1)
	    chNum = metaDataVector.size();

	// check to see if we need to grow the vector
	if (chNum >= metaDataVector.size())
	    metaDataVector.setSize(chNum + 1);
	...
	return (ArgArray) metaData.get(namesToChan.get(new Integer(channelNo))
		.toString());
