...
public class WrappersUtil {
  ...
  public static HashMap<String, Class<?>> loadWrappers(HashMap<String, Class<?>> wrappers) throws ClassNotFoundException{
    ...
    try {// Trying to load the wrapper specified in the configuration file of the container. 
      config = new PropertiesConfiguration ( Main.DEFAULT_WRAPPER_PROPERTIES_FILE );
    } catch ( ConfigurationException e ) {
      logger.error ( "The wrappers configuration file's syntax is not compatible." );
      logger.error ( new StringBuilder ( ).append ( "Check the :" ).append ( Main.DEFAULT_WRAPPER_PROPERTIES_FILE ).append ( " file and make sure it's syntactically correct." ).toString ( ) );
    ...
    for ( int i = 0 ; i < wrapperNames.length ; i++ ) {
      ...
      if ( wrappers.get ( name ) != null ) {
        ...
        logger.error ( new StringBuilder ( ).append ( "Please check the " ).append ( Main.DEFAULT_WRAPPER_PROPERTIES_FILE ).append ( " file and try again." ).toString ( ) );
        System.exit ( 1 );
      }
