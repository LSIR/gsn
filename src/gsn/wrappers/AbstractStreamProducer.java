...
public abstract class AbstractStreamProducer extends Thread implements StreamProducer {
   ...
   public boolean initialize ( TreeMap initialContext ) {
      ...
      if ( isDebugEnabled == true ) logger.debug( new StringBuilder( ).append( "Initializing connection to " ).append( host ).append( ":" ).append( port ).toString( ) );
   ...
   class TableSizeEnforce implements Runnable {
            ...
            synchronized ( AbstractStreamProducer.this.listeners ) {
               ...
               if ( isCacheNeedsUpdate ) {
                  ...
                  for ( DataListener listener : listeners ) {
                     if ( listener.getStreamSource( ).getParsedStorageSize( ) != StreamSource.STORAGE_SIZE_NOT_SET ) deleteStatement.append(
                  ...
                  if ( isDebugEnabled == true ) logger.debug( new StringBuilder( ).append( "RESULTING QUERY FOR Table Size Enforce " ).append( deleteStatement.toString( ) ).toString( ) );
                  isCacheNeedsUpdate = false;
               }
            }
            int resultOfUpdate = StorageManager.getInstance( ).executeUpdate( deleteStatement );
            if ( isDebugEnabled == true ) logger.debug( new StringBuilder( ).append( resultOfUpdate ).append( " old rows dropped from " ).append( tableName ).toString( ) );
         }
      }
      ...
      private StringBuilder extractConditions ( boolean countBased , int historySize , Date startDate , Date endDate ) {
         ...
         if ( StorageManager.isHsql( ) ) {
            ...
            if ( countBased ) result.append( "( ( " ).append( tableName ).append( ".TIMED <= ( SELECT * from ( SELECT TIMED from " ).append( tableName ).append(
               " group by TIMED order by TIMED desc limit 1 offset " ).append( historySize ).append( "  ) AS TMP_" ).append( ( int ) ( Math.random( ) * 10000000 ) ).append(
               " ) ) AND (" + tableName + ".TIMED >=" + startDate.getTime( ) + ") AND (" + tableName + ".TIMED<=" + endDate.getTime( ) + ") )" );
            else
