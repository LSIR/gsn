...
public abstract class AbstractStreamProducer extends Thread implements StreamProducer {
    ...
    public boolean initialize(TreeMap initialContext) {
        ...
        if (port <= 0) {
            logger.error(new StringBuilder().append("The port ").append(port).append(" is invalid for a stream source.").toString());
            return false;
        }
        if (isDebugEnabled == true)
            logger.debug(new StringBuilder().append("Initializing connection to ").append(host).append(":").append(port).toString());
        activeAddressBean = (AddressBean) initialContext.get(Container.STREAM_SOURCE_ACTIVE_ADDRESS_BEAN);

        this.storageMan = (StorageManager) initialContext.get(VSensorLoader.STORAGE_MANAGER);
        if (storageMan == null) {
            logger.fatal("The Storage Manager shouldn't be null, possible a BUG.");
            return false;
        }
        if (activeAddressBean == null) {
            logger.fatal("The address bean supplied to initializer shouldn't be null, possible a BUG.");
            return false;
        }
    ...
    public String addListener(DataListener dataListener) {
        ...
        if (isDebugEnabled == true)
            logger
                    .debug(new StringBuilder().append("The view name=").append(viewName).append(" with the query=").append(resultQuery).toString());
    ...
    class TableSizeEnforce implements Runnable {
         ...
         * which are not interested by any of the existing listeners. The query
         * which is used for dropping the unused stream elements is going to be
         * generated whever there is a change in the set of registered
         * DataListeners. If there is a change the <code>isCacheNeedsUpdate</code>
         * will be true which triggers the thread to generate a new query for
         * removing the unused stream elements.
         */
                    ...
                    if (isCacheNeedsUpdate) {
                        ...
                        for (DataListener listener : listeners) {
                            if (listener.getStreamSource().getParsedStorageSize() != StreamSource.STORAGE_SIZE_NOT_SET)
                                deleteStatement.append(extractConditions(listener.isCountBased(), listener.getHistorySize(), listener
                                        .getStreamSource().getStartDate(), listener.getStreamSource().getEndDate())).append("  AND ");
                        }
                        ...
                        if (isDebugEnabled == true)
                            logger.debug(new StringBuilder()
                                    .append("RESULTING QUERY FOR Table Size Enforce ").append(deleteStatement.toString()).toString());
                        isCacheNeedsUpdate = false;
                    }
                }
                int resultOfUpdate = StorageManager.getInstance().executeUpdate(deleteStatement);
                if (isDebugEnabled == true)
                    logger.debug(new StringBuilder().append(resultOfUpdate).append(" old rows dropped from ").append(tableName).toString());
            }
        }
        ...
        private StringBuilder extractConditions(boolean countBased, int historySize, Date startDate, Date endDate) {
            ...
            if (StorageManager.isHsql()) {
                if (countBased)
                    // result.append ( "( ( " ).append ( tableName ).append ( ".TIMED
                    // not in ( select " ).append ( tableName ).append ( ".TIMED from
                    // " ).append (
                    // tableName ).append ( " order by " ).append ( tableName
                    // ).append ( ".TIMED DESC LIMIT " ).append ( historySize
                    // ).append (
                    // " offset 0 )) AND (" + tableName + ".TIMED >=" +
                    // startDate.getTime () + ") AND (" + tableName + ".TIMED<=" +
                    // endDate.getTime () + ") )" );
                    result
                            .append("( ( ").append(tableName).append(".TIMED <= ( SELECT TIMED from ").append(tableName)
                            .append(" group by TIMED order by TIMED desc limit 1 offset ").append(historySize).append("  ) ) AND (" + tableName + ".TIMED >="
                            + startDate.getTime() + ") AND (" + tableName + ".TIMED<=" + endDate.getTime() + ") )");
                else
                    result.append("( ( ").append(tableName).append(".TIMED < (NOW_MILLIS() -").append(historySize).append(") ) AND (" + tableName
                            + ".TIMED >=" + startDate.getTime() + ") AND (" + tableName + ".TIMED<=" + endDate.getTime() + ") )");
            } else if (StorageManager.isMysqlDB()) {
                if (countBased)
                    result
                            .append("( ( ").append(tableName).append(".TIMED <= ( SELECT * from ( SELECT TIMED from ").append(tableName)
                            .append(" group by TIMED order by TIMED desc limit 1 offset ").append(historySize).append("  ) AS TMP_").append((int) (Math
                            .random() * 10000000)).append(" ) ) AND (" + tableName + ".TIMED >=" + startDate.getTime() + ") AND (" + tableName
                            + ".TIMED<=" + endDate.getTime() + ") )");
                else
                    result.append("( ( ").append(tableName).append(".TIMED < (UNIX_TIMESTAMP() -").append(historySize).append(") ) AND (" + tableName
                            + ".TIMED >=" + startDate.getTime() + ") AND (" + tableName + ".TIMED<=" + endDate.getTime() + ") )");
            }
    ...
    protected void publishData(StreamElement streamElement) {
        ...
        if (result == false) {
            ...
            synchronized (listeners) {
                if (listeners.size() == 0)
                    logger.warn("A wrapper without listener shouldn't exist. !!!");
                for (Iterator<DataListener> iterator = listeners.iterator(); iterator.hasNext();) {
                    ...
                    if (results) {
                        if (isDebugEnabled == true)
                            logger.debug("Output stream produced/received from a wrapper");
