...
public class DummyDataProducer extends AbstractStreamProducer {
    ...
    /**
         ...
         * If the underlying sensor network is data-driven (e.g., the sensor
         * network doesn't have a specific rate)<br>
         * you can check the <code>gsn.wrappers.SFWrapperDS</code> which is
         * the wrapper implemented for the serial forward software.<br>
    ...
    /**
         ...
         * before any real task to be done. <p/> The initalContext which can be
         * used to extract required information. The storageManager useful for
         * issuing the queries.
         */
	...
	if (addressBean.getPredicateValue("packet-size") != null) {
	    dataSizeInBytes = ParamParser.getInteger(addressBean
		    .getPredicateValue("packet-size"),
		    DATA_SIZE_IN_BYTES_DEFAULT_VALUE);
	    if (dataSizeInBytes <= 0) {
		logger
			.warn("The specified >packet-size< parameter for the >DummyDataProducer< shouldn't be a negative number.\nGSN uses the default size("
				+ DATA_SIZE_IN_BYTES_DEFAULT_VALUE + " bytes).");
		dataSizeInBytes = DATA_SIZE_IN_BYTES_DEFAULT_VALUE;
	    }

	}
	if (addressBean.getPredicateValue("rate") != null) {
	    rate = ParamParser.getInteger(
		    addressBean.getPredicateValue("rate"), RATE_DEFAULT_VALUE);
	    if (rate <= 0) {
		logger
			.warn("The specified rate parameter for the >DummyDataProducer< should be a positive number.\nGSN uses the default rate ("
				+ RATE_DEFAULT_VALUE + ").");
	...
	int counter = 0;
	/**
         * The DummyDataProducer looks for clients. If there is no client
         * register, it sleeps and
         */
	...
	while (isActive()) {
	    ...
	    if (dataSizeInBytes > 0)
		streamElement = new StreamElement(new String[] { "DATA",
			"DATA_BIN" }, new Integer[] { DataTypes.INTEGER,
			DataTypes.BINARY }, new Serializable[] { counter,
			binaryPortionOfEachStreamElement }, System
			.currentTimeMillis());
	    else
		streamElement = new StreamElement(new String[] { "DATA" },
			new Integer[] { DataTypes.INTEGER },
			new Serializable[] { counter }, System
				.currentTimeMillis());
    ...
    /**
         * This Method expresses to the GSN Container the output structure of
         * the wrapper. <br>
         * For instance, the following implementation shows that each stream
         * element produced by<br>
         * the wrapper contains two fields, first one called <code>DATA</code>
         * and the<br>
