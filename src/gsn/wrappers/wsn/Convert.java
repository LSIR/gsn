...
public class Convert {
	...
	// adc Voltage = 473
	System.out.println("\nHere is the voltage : " + cv.convertVoltage(473)
		+ " mV\n");

	// Light
	double light = cv.convertLight(913, cv.convertVoltage(473));
	System.out.println("Here is the light value : "
		+ cv.readableValue(light) + "\n");

	// Temp
	double kelvinTemp = cv.convertTemprature(508);
	System.out.println("The temp value is : "
		+ cv.readableValue(kelvinTemp) + " Kelvin degres");
	System.out.println("The temp value is : "
		+ cv.readableValue((float) (kelvinTemp - 273.15))
		+ " Celsus degres\n");

	// Magnometer magX
	double magX = cv.convertMag(804);
	System.out.println("Here is the mag X value : "
		+ cv.readableValue(magX) + " mGauss");

	// Magnometer magY
	double magY = cv.convertMag(805);
	System.out.println("Here is the mag Y value : "
		+ cv.readableValue(magY) + " mGauss\n");
	...
	System.out.println("Here is the accel X value : "
		+ cv.readableValue(accelX));

	// AccelY converting
	double accelY = cv.convertAccelY(492);
	System.out.println("Here is the accel Y value : "
		+ cv.readableValue(accelY) + "\n");
	...
	double reading = (zero_value - (plus_one_calibrationX - adcData))
		/ zero_value;
	...
	double reading = (zero_value - (plus_one_calibrationY - adcData))
		/ zero_value;
	return reading;
