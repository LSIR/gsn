...
public class WSNSWrapper extends AbstractStreamProducer implements DataListener {
    ...
    /**
     * The rate, set the rate in which the network is re-revaluated.
     * If the rate is 1000, then the network is reevaluated every seconds.
     * If the rate is negative, the network will stop evaluating it self after specified number of cycles.
     */
        ...
        while (isActive()) {
            if (tempStepCounter <= step_counter || step_counter == -1) {
                ...
                if (! listeners.isEmpty() && dataBuffer.size() > 0) {
                    ...
                    StreamElement streamElement = new StreamElement(new String [ ]{"NODE_ID", "PARENT_ID", "TEMPREATURE"}, new Integer [ ]{DataTypes.INTEGER,
                            DataTypes.INTEGER, DataTypes.INTEGER}, new Serializable [ ]{dataPacket.getIdentifier(), dataPacket.getParent(),
                            dataPacket.getValue()}, System.currentTimeMillis());
    ...
    public WirelessNode [ ] initializeNodes(int nodeCount) throws RuntimeException {
        if (nodeCount <= 0)
            throw new RuntimeException("Wireless Sensor Network Simulator (WSNS) can't create a network with zero or negative number of nodes : " + nodeCount);
