...
public class WirelessNode extends Thread implements WirelessNodeInterface {
	...
	while (alive) {
	    ...
	    DataPacket dataPacket = new DataPacket(identity,
		    (parent == null) ? -1 : parent.getIdentifier(),
		    tempreature, DataPacket.ROUTING_AND_DATA_PACKET);
	    send(dataPacket);
	}
    }
	...
	if (parent != null) {
		...
		dataListener.newDataAvailable(e);
	    else {
		System.out
			.println("Data received by the wireless node with the id of = "
				+ identity);
	    }
	}
    }
