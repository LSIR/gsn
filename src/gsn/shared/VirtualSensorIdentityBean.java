...
public class VirtualSensorIdentityBean {
	...
	Enumeration<String> values = req
		.getHeaders(Registry.VS_PREDICATES_VALUES);
	...
	nextRequirement: for (KeyValue askedPredicate : input) {
		...
		if (((String) myPredicates.getKey())
			.equalsIgnoreCase((String) askedPredicate.getKey()))
		    if (((String) myPredicates.getValue())
			    .equalsIgnoreCase((String) askedPredicate
				    .getValue()))
	...
	if (obj == null || !(obj instanceof VirtualSensorIdentityBean))
	    return false;
	VirtualSensorIdentityBean input = (VirtualSensorIdentityBean) obj;
	return (input.name.equals(name)
		&& input.remoteAddress.equals(remoteAddress) && (input.remotePort == remotePort));
    }
