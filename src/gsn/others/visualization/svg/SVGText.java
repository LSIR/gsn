...
public class SVGText extends AbstractSVGElement implements LayoutEntity {
	...
	this.cachedHeight = cachedDimention.getTotalHeightUpTo(cachedDimention
		.getHeights().length) + 5;
	...
	if (!isEnable())
	    return;
	SVGRectangle svgRectangle = new SVGRectangle("", getXInLayout(),
		getYInLayout(), getWidthInLayout(), getHeightInLayout());
	...
	for (int i = 0; i < tokens.length; i++) {
	    ...
	    stringTemplate
		    .setAttribute(
			    ...
			    getYInLayout()
				    + (cachedDimention
					    .getTotalHeightUpTo(i + 1) - cachedDimention
					    .getDecendents()[i]) + 1);
	    stringTemplate.setAttribute("FONT_SIZE", fontSize);
	    stringTemplate.setAttribute("FILL_OPACITY", opaque);
	    stringTemplate.setAttribute("FILL_COLOR", SVGUtils
		    .formatToHex(fillColor.getRGB()));
