...
public class SVGUtils {
	...
	StringBuilder toReturn = new StringBuilder(Integer
		.toHexString(rgb & 0xffffff));
    ...
    public static void performLayout(
	    Collection<? extends LayoutEntity> entities,
	    Collection<? extends LayoutRelationship> relationships,
	    LayoutAlgorithm currentLayoutAlgorithm, int width, int height) {
	try {
		...
		if (!entities.contains(rel.getSourceInLayout())
			|| !entities.contains(rel.getDestinationInLayout()))
		    throw new RuntimeException(
			    "The is a node in the relations that is not included in the list of the nodes.");
	    }
	    currentLayoutAlgorithm.applyLayout(layoutEntities,
		    layoutRelationships, 0, 0, width, height, false, false);
	} catch (InvalidLayoutConfiguration e) {
	    System.out.println("Not a valid layout configuration:\nlayout='"
		    + currentLayoutAlgorithm.toString() + "', continuous='"
		    + false + "', asynchronous='");
    ...
    public static Width_Height_Decendent_ValueBean getBoundsOfAString(String s,
	    Font f) {
	int width = 0;
	s = s.replace("\t", "   ");
	...
	return new Width_Height_Decendent_ValueBean(tokens, width, heights,
		decendents);
    }
}
