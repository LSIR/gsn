...
public class SVGCircle extends AbstractSVGElement implements LayoutEntity {
    ...
    public SVGCircle(Object internalObject, int x, int y, int radios,
	    Color color) {
	this.realObject = internalObject;
	this.userX = x;
	this.userY = y;
	...
	st.reset();
	st.setAttribute("X", getXInLayout()
		+ Math.min(getWidthInLayout(), getHeightInLayout()) / 2);
	st.setAttribute("Y", getYInLayout()
		+ Math.min(getWidthInLayout(), getHeightInLayout()) / 2);
	st.setAttribute("R",
		Math.min(getWidthInLayout(), getHeightInLayout()) / 2);
	st.setAttribute("FILL_COLOR", SVGUtils.formatToHex(fillColor.getRGB()));
	st.setAttribute("STROKE_COLOR", SVGUtils.formatToHex(borderColor
		.getRGB()));
