...
public abstract class AbstractSVGElement implements SVGElement {
	...
	if (link != null)
	    input.append(SVGUtils.hyperLinkingAddress(link));
	...
	int size = listOfRels.size();
	SVGEdge[] a_listOfRels = (SVGEdge[]) this.listOfRels
		.toArray(new SVGEdge[size]);
	...
	for (int i = 0; i < a_listOfRels.length; i++) {
		...
		throw new RuntimeException(
			"Problem, we have a relationship and we are not the source or the dest");
	    }
