...
public class NotificationUtils {
    ...
    /**
         * Extracts the prefix of the address url of the notification. If it
         * doesn't defined, it returns zero.
         * 
         * @param url
    ...
    /**
         ...
         * @param message
         */
    public static NotificationRequest getProcessedQuery(String address,
	...
	if (protocol == 0) {
	    ...
	    if (logger.isInfoEnabled())
		logger
			.info("The address path of the queries doesn't contain recognized protocol");
	}
	NotificationRequest notificationRequest = null;
	switch (protocol) {
	case NotificationUtils.SMS:
	    notificationRequest = new SMSNotification(NotificationUtils
		    .extractAddress(address).trim(), query, message);
	    break;
	case NotificationUtils.EMAIL:
	    notificationRequest = new EmailNotification(NotificationUtils
		    .extractAddress(address), query, message);
	    break;
    ...
    public static String prepareMessage(StreamElement streamElement,
	    String message) {
	...
	for (int i = 0; i < fieldNames.length; i++)
	    template.setAttribute(streamElement.getFieldNames()[i],
		    streamElement.getData()[i]);
