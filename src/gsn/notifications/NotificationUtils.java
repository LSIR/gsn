...
public class NotificationUtils {
    ...
    * 
    * @param url of the notification address.
    */
   public static int getProtocol ( String url ) {
      if ( url == null ) return 0;
      String toUpper = url.trim( ).replace( " " , "" ).toUpperCase( );
      if ( toUpper.startsWith( SMS_PROTOCOL ) ) return SMS;
      else if ( toUpper.startsWith( PDA_PROTOCOL ) ) return PDA;
      else if ( toUpper.startsWith( EMAIL_PROTOCOL ) ) return EMAIL;
      else if ( toUpper.startsWith( PHONE_PROTOCOL ) ) return PHONE;
      else if ( toUpper.startsWith( GSN_PROTOCOL ) ) return GSN;
      else if ( toUpper.startsWith( PAGER_PROTOCOL ) ) return PAGER;
      else if ( toUpper.startsWith( FAX_PROTOCOL ) ) return FAX;
      return 0;
   }
   ...
   /**
    * @param address The full address (incl. the protocol prefix).
    * @param query
    * @param message
    */
   public static NotificationRequest getProcessedQuery ( String address , String query , String message ) {
      ...
      if ( protocol == 0 ) {
         if ( logger.isInfoEnabled( ) ) logger.info( "BAD QUERY RECEIVED" );
         if ( logger.isInfoEnabled( ) ) logger.info( "The address path of the queries doesn't contain recognized protocol" );
      }
