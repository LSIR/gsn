 ...
 *         Note that : 1. Set the REQUEST HEADER PARAMETER DESCRIBING THE BODY
 *         OF THE REQUEST. 2. ONCE YOU SET THE BODY AS STREAM THE REQUEST
 *         PARAMETER DOESN'T WORK HENCE USE THE REQUEST-HEADER-PARAMETER.
 */
public class GSNNotification extends NotificationRequest {
	...
	this.queryWithoutDoubleQuots = new StringBuilder(req.getHeader(
		Container.VS_QUERY).trim().replace("\"", ""));
    }
	...
	while (data.hasMoreElements()) {
	    se = data.nextElement();
	    int result = notifyPeerAboutData(getRemoteAddress(),
		    getRemotePort(), getNotificationCode(), se);
	    if (result != 0 && result != 200) {
		logger.warn(new StringBuilder().append(
			"The result of HTTP response was :").append(result)
			.append(" ,Thus notification failed").toString());
		return false;
	    }
	}
	return true;
    ...
    private int notifyPeerAboutData(String host, int port,
	    String notificationCode, Serializable data) {
	...
	try {
	    if (statusCode != -1)
		if (postMethod.getResponseHeader(Container.RES_STATUS)
    ...
    public static ByteArrayInputStream InputStreamAdapter(Serializable so)
	    throws IOException {
