 ...
 * Note that :
 * 1. Set the REQUEST HEADER PARAMETER DESCRIBING THE BODY OF THE REQUEST.
 * 2. ONCE YOU SET THE BODY AS STREAM THE REQUEST PARAMETER DOESN'T WORK HENCE USE THE REQUEST-HEADER-PARAMETER.
 */
public class GSNNotification extends NotificationRequest {
      ...
      while ( data.hasMoreElements ( ) ) {
         ...
         if ( result != 0 && result != 200 ) {
            logger.warn ( new StringBuilder ( )
                  .append ( "The result of HTTP response was :" ).append ( result ).append ( " ,Thus notification failed" ).toString ( ) ) ;
            return false ;
         }
      }
      return true ;
   ...
   private int notifyPeerAboutData ( String host , int port , String notificationCode , Serializable data ) {
      ...
      try {
         if ( statusCode != - 1 )
            if ( postMethod.getResponseHeader ( Container.RES_STATUS ).getValue ( ).equals ( Container.INVALID_REQUEST ) )
               statusCode = - 10 ;
