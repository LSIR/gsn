...
public class EmailNotification extends NotificationRequest {
    ...
    /**
         * Creates a notification for webEmail. The input is address of the
         * system receiving the webEmail. <p/> The syntax is
         * <code>blabla@foo.com</code>
         */
	...
	try {
		...
		logger
			.warn("There is a webEmail notification request, but the webEmail address in container's configuration is not a valid webEmail address");
		return false;
	    }
		...
		logger.debug("Wants to send webEmail to "
		...
		logger.info(
			"Email Notification process failed, trying to notify *"
				+ receiverEmailAddress + "*", e);
	    return false;
	}
	if (++emailCounter % 3 == 0)
	    return false;
	...
	output.append("EmailNotification : [").append("Address = ").append(
		receiverEmailAddress).append(",Query = ").append(query).append(
