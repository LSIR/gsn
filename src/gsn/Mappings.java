...
public final class Mappings {
    ...
    private static final TreeMap<String, VSensorConfig> vsNameTOVSConfig = new TreeMap<String, VSensorConfig>(
	    new CaseInsensitiveComparator());

    private static final TreeMap<String, VSensorInstance> fileNameToVSInstance = new TreeMap<String, VSensorInstance>(
	    new CaseInsensitiveComparator());
	...
	try {
		...
		logger.info((new StringBuilder("Testing the pool for :"))
			.append(instance.getConfig().getVirtualSensorName())
			.toString());
	    ...
	    logger
		    .error("GSN can't load the virtual sensor specified at "
	...
	TreeMap<String, Boolean> vsNameToOutputStructureFields = new TreeMap<String, Boolean>(
		new CaseInsensitiveComparator());
	vsNamesToOutputStructureFields.put(instance.getConfig()
		.getVirtualSensorName(), vsNameToOutputStructureFields);
	for (DataField fields : instance.getConfig().getOutputStructure())
	    vsNameToOutputStructureFields.put(fields.getFieldName(),
		    Boolean.TRUE);
	vsNameToOutputStructureFields.put("TIMED", Boolean.TRUE);
	vsNameTOVSConfig.put(instance.getConfig().getVirtualSensorName(),
		instance.getConfig());
    ...
    public static final TreeMap<String, Boolean> getVsNamesToOutputStructureFieldsMapping(
	    String vsName) {
