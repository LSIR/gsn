...
public class ContainerImpl extends HttpServlet implements Container {
    ...
    /**
         ...
         * that recently produced data. This variable is useful for batch
         * processing TIMED couple virtual sensor produce data.
         */
    /*
	...
	int requestType = -1;
	try {
	    requestType = Integer.parseInt((String) req
	...
	if (requestType == Container.LIST_VIRTUAL_SENSORS) {
	    ...
	    res.setHeader(Container.RES_STATUS,
	...
	String prespectiveVirtualSensor = req
	...
	if (requestType == Container.ONE_SHOT_QUERY_EXECUTION_REQUEST) {
		...
		res.setHeader(Container.RES_STATUS, Container.INVALID_REQUEST);
		logger
			.info("A request for an execution received without a query ?!!");
		return;
	    }
	    res.setHeader(Container.RES_STATUS,
		    Container.REQUEST_HANDLED_SUCCESSFULLY);
	    Enumeration<StreamElement> rs = StorageManager.getInstance()
		    .executeQuery(new StringBuilder(query));
