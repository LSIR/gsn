...
public class ContainerImpl extends HttpServlet implements Container {
   ...
   public void publishData ( VirtualSensor sensor ) {
      ...
      if ( logger.isDebugEnabled( ) ) logger.debug( new StringBuilder( ).append( "There are queries " ).append( registered.size( ) ).append( " registered for >" ).append( name ).append( "<" )
            .toString( ) );
      ...
      synchronized ( registered ) {
         ...
         while ( registeredIterator.hasNext( ) ) {
            ...
            if ( logger.isDebugEnabled( ) ) logger.debug( new StringBuilder( ).append( "Evaluating INTERESTED QUERY " ).append( " : " ).append( query ).append( ">>> NEEDS NOTIFICATION = " ).append(
               interestedClient.needNotification( ) ).toString( ) );
            if ( interestedClient.needNotification( ) ) notificationCandidates.add( interestedClient );
      ...
      if ( requestType == Container.REGISTER_PACKET || requestType == Container.DEREGISTER_PACKET ) {
         ...
         if ( requestType == Container.REGISTER_PACKET ) {
            ...
            if ( logger.isInfoEnabled( ) ) logger.info( new StringBuilder( ).append( "REGISTER REQUEST FOR " ).append( prespectiveVirtualSensor ).append( " received from :" ).append(
               interest.getRemoteAddress( ) ).append( ":" ).append( interest.getRemotePort( ) ).append( " under the code " ).append( interest.getNotificationCode( ) ).append( " With the query of *" )
                  .append( interest.getQuery( ) ).append( "*" ).toString( ) );
   ...
   public void addRemoteStreamSource ( String notificationCode , RemoteDS remoteDS ) {
      ...
      if ( logger.isDebugEnabled( ) ) logger.debug( new StringBuilder( ).append( "Remote DataSource DBALIAS *" ).append( remoteDS.getDBAlias( ) ).append( "* with the code : *" ).append(
         notificationCode ).append( "* added." ).toString( ) );
   }
