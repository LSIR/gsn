...
class ChartInfo {
    ...
    public synchronized void addData(StreamElement streamElement) {
        ...
        for (int i = 0; i < streamElement.getFieldNames().length; i ++) {
            ...
            if (timeSeries == null) {
                dataForTheChart.put(streamElement.getFieldNames()[i], timeSeries = new TimeSeries(
            ...
            try {
                timeSeries.add(new FixedMillisecond(new Date(streamElement.getTimeStamp())), Integer.parseInt(streamElement.getData()[i]
                        .toString()));
