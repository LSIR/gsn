...
public class ChartVirtualSensor extends AbstractVirtualSensor {
    ...
    private static final transient Logger logger = Logger
	    .getLogger(ChartVirtualSensor.class);

    /**
         * The <code>GENERATE_COUNT</code> represents after how many inputs,
         * the virtual sensor should generate data. By default it set to 1
         * meaning that for each stream element received, the virtual sensor
         * plots a new diagram. If you want to make the virtual sensor plot
         * after receiving each K stream elements, set
         * <code>GENERATE_COUNT</code> to K.
         */
    private final int GENERATE_COUNT = 4;

    private long counter = 0;

    private final static TreeMap<String, ChartInfo> input_stream_name_to_ChartInfo_map = new TreeMap<String, ChartInfo>(
	...
	/**
         ...
         */
	if (super.initialize(map) == false)
	    return false;
	TreeMap<String, String> params = virtualSensorConfiguration
	...
	chartInfo.setHistorySize(ParamParser.getInteger(params
		.get("history-size"), 10));
	input_stream_name_to_ChartInfo_map.put(chartInfo.getInputStreamName(),
		chartInfo);
	...
	if (logger.isDebugEnabled())
	    logger.debug(new StringBuilder("data received under the name *")
		    .append(inputStreamName).append("* to the ChartVS.")
		    .toString());
	/**
         * Finding the appropriate ChartInfo object for this input stream.
         */
	ChartInfo chartInfo = input_stream_name_to_ChartInfo_map
		.get(inputStreamName);
	/**
         * If there is not chartInfo configured for this input stream, the
         * virtual sensor doesn't produce any values. Note that if this virtual
         * sensor is intended to produce output other than plots (e.g., if
         * output of this virtual sensor also container integers), then one
         * might comment the following line.
         */

	if (chartInfo == null) {
	    logger
		    .warn("ChartVS drops the input because there is no chart specification defined for the specific input.");
	    return;
	}
	/**
	...
	String[] fieldNames = input_stream_name_to_ChartInfo_map.keySet()
		.toArray(new String[] {});
	Integer[] fieldTypes = new Integer[fieldNames.length];
	Serializable[] charts = new Serializable[fieldNames.length];
	for (int i = 0; i < fieldTypes.length; i++) {
	    /**
                 * We set the type of the output stream element to Types.Binary
                 * because we are producing images.
                 */
	...
	/**
         * In here our stream element's relation contains just one row of data
         * and it's filled using the binary data which contains the plots. Note
         * that this virtual sensor plots one diagram for each InputStreamName.
         * Also Note that, each InputStreamName can have one or more variables
         * inside it's stream elements's relation thus having one plot for
         * several variables.
         */

	for (int i = 0; i < fieldNames.length; i++) {
	    ChartInfo chart = input_stream_name_to_ChartInfo_map
		    .get(fieldNames[i]);
	    charts[i] = chart.writePlot().toByteArray();
	}
	StreamElement output = new StreamElement(fieldNames, fieldTypes,
		charts, System.currentTimeMillis());
...
class ChartInfo {
	...
	if (!ready) {
	    chart = ChartFactory.createTimeSeriesChart(plotTitle, "Time",
		    verticalAxisTitle, dataCollectionForTheChart, true, true,
		    false);
    ...
    /**
         * This method adds the specified stream elements to the timeSeries of
	...
	for (int i = 0; i < streamElement.getFieldNames().length; i++) {
		...
		dataForTheChart.put(streamElement.getFieldNames()[i],
			timeSeries = new TimeSeries(streamElement
				.getFieldNames()[i],
	    ...
	    try {
		timeSeries.add(new FixedMillisecond(new Date(streamElement
			.getTimeStamp())), Integer.parseInt(streamElement
			.getData()[i].toString()));
	...
	byteArrayOutputStream.reset();
	try {
	    ChartUtilities.writeChartAsPNG(byteArrayOutputStream, chart, width,
		    height, false, 8);
	...
	try {
		...
		buffer.append("Input-Stream Name : ").append(inputStreamName)
			.append("\n");
	    }
