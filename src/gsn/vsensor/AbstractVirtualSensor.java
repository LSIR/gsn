...
public abstract class AbstractVirtualSensor implements VirtualSensor {
   ...
   public boolean initialize ( HashMap map ) {
      ...
      if ( isInfoEnabled ) logger.info( ( new StringBuilder( "Initialization of :" ) ).append( virtualSensorConfiguration.getVirtualSensorName( ) ).toString( ) );
      return true;
   }
   ...
   private void validateStreamElement ( StreamElement streamElement ) {
      if ( !compatibleStructure( streamElement.getFieldTypes( ) , virtualSensorConfiguration.getOutputStructure( ) ) ) {
         ...
         exceptionMessage.append( "The expected stream element structure (specified in " ).append( virtualSensorConfiguration.getFileName( ) ).append( " is [" );
         ...
         exceptionMessage.append( "] but the actual stream element received from the " + virtualSensorConfiguration.getVirtualSensorName( ) ).append( " has the [" );
   ...
   protected synchronized void dataProduced ( StreamElement streamElement ) {
      ...
      if ( ( currentTime - lastVisitiedTime ) < outputStreamRate ) {
         if ( isInfoEnabled ) logger.info( "Called by *discarded* b/c of the rate limit reached." );
         return;
      }
   ...
   public void finalize ( HashMap map ) {
      if ( isInfoEnabled ) logger.info( ( new StringBuilder( "Finalizing the :" ) ).append( virtualSensorConfiguration.getVirtualSensorName( ) ).append( "." ).toString( ) );
   }
