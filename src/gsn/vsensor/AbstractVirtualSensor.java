...
public abstract class AbstractVirtualSensor implements VirtualSensor {
    ...
    private static final transient Logger logger = Logger
	    .getLogger(AbstractVirtualSensor.class);
	...
	virtualSensorConfiguration = (VSensorConfig) map
		.get(VirtualSensorPool.VSENSORCONFIG);
	if (isInfoEnabled)
	    logger.info((new StringBuilder("Initialization of :")).append(
		    virtualSensorConfiguration.getVirtualSensorName())
		    .toString());
	return true;
    }

    protected void validateStreamElement(StreamElement streamElement) {
	if (!compatibleStructure(streamElement.getFieldTypes(),
		virtualSensorConfiguration.getOutputStructure())) {
	    StringBuilder exceptionMessage = new StringBuilder()
		    .append("The streamElement produced by :")
		    .append(virtualSensorConfiguration.getVirtualSensorName())
		    .append(
			    " Virtual Sensor is not compatible with the defined streamElement.\n");
	    exceptionMessage.append(
		    "The expected stream element structure (specified in ")
		    .append(virtualSensorConfiguration.getFileName()).append(
			    " is [");
	    for (DataField df : virtualSensorConfiguration.getOutputStructure()) {
		exceptionMessage.append(df.getFieldName()).append(" (").append(
			DataTypes.TYPE_NAMES[df.getDataTypeID()])
			.append(") , ");
	    }
	    exceptionMessage
		    .append(
			    "] but the actual stream element received from the "
				    + virtualSensorConfiguration
					    .getVirtualSensorName()).append(
			    " has the [");
	    for (int i = 0; i < streamElement.getFieldNames().length; i++)
		exceptionMessage.append(streamElement.getFieldNames()[i])
			.append("(").append(
				DataTypes.TYPE_NAMES[streamElement
					.getFieldTypes()[i]]).append("),")
			.append(" ] thus the stream element dropped !!!");
	...
	final int outputStreamRate = virtualSensorConfiguration
		.getOutputStreamRate();
    ...
    private static boolean compatibleStructure(Integer[] fieldTypes,
	    ArrayList<DataField> outputStructure) {
	if (outputStructure.size() != fieldTypes.length) {
	    logger
		    .warn("Validation problem, the number of field doesn't match the number of output data strcture of the virtual sensor");
	    return false;
	}
	for (int i = 0; i < outputStructure.size(); i++) {
	    if (fieldTypes[i] != outputStructure.get(i).getDataTypeID()) {
		logger
			.warn("Validation problem for output field >"
				+ outputStructure.get(i).getFieldName()
				+ ", The field type declared as >"
				+ DataTypes.TYPE_NAMES[fieldTypes[i]]
				+ "< within the stream element but it is defined as in the VSD as : "
				+ DataTypes.TYPE_NAMES[outputStructure.get(i)
					.getDataTypeID()]);
		return false;
	    }
	}
	...
	if (isInfoEnabled)
	    logger.info((new StringBuilder("Finalizing the :")).append(
		    virtualSensorConfiguration.getVirtualSensorName()).append(
		    ".").toString());
    }
