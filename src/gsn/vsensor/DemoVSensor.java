...
public class DemoVSensor extends AbstractVirtualSensor {
	...
	if (inputStreamName.equalsIgnoreCase("CSTREAM")) {
		...
		break;
	    default:
		logger.warn(new StringBuilder().append("Shouldn't happen.>")
			.append(counter).append("<").toString());
	    }
	    ...
	    StreamElement outputSE = new StreamElement(OUTPUT_FIELDS,
		    OUTPUT_TYPES, new Serializable[] { outputStream
			    .toByteArray() }, data.getTimeStamp());
