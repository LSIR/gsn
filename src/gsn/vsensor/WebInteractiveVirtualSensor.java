...
public class WebInteractiveVirtualSensor extends AbstractVirtualSensor {
    ...
    private static final transient Logger logger = Logger
	    .getLogger(WebInteractiveVirtualSensor.class);
	...
	int memorySizeInSeconds = ParamParser.getInteger(params
		.get("memory-size-in-seconds"), -1);
	if (memorySizeInSeconds == -1) {
	    logger
		    .error("The parameter *memory-size-in-seconds* is missing from the virtual sensor processing class's initialization.");
	...
	if (inputStreamName.equalsIgnoreCase(INPUT_STREAM_NAME)) {
	    ...
	    SVGCircle simpleNodeObject = new SVGCircle(new SimpleNodeObject(
		    node_id, parent_id, SimpleNodeObject.REQUEST_TYPE_DATA,
		    tempreature), Color.blue);
	...
	StreamElement out = new StreamElement(
		new String[] { OUTPUT_FIELD_NAME },
		new Integer[] { DataTypes.BINARY },
		new Serializable[] { visualizedResults }, System
			.currentTimeMillis());
	dataProduced(out);
	...
	try {
	    virtualSensorConfiguration.getInputStream(INPUT_STREAM_NAME)
		    .getSource(streamSourceAliasName).getActiveSourceProducer()
		    .sendToWrapper(data);
	} catch (OperationNotSupportedException e) {
	    logger
		    .warn(new StringBuilder()
			    .append("The virtual sensor : ")
			    .append(
				    virtualSensorConfiguration
					    .getVirtualSensorName())
			    .append(
				    " want to send data to a stream source which doesn't support receiving data.")
			    .toString());
	    logger.warn(e.getMessage(), e);
	}
    }

    public static String visualaize(int width, int height,
	    ArrayList<SVGCircle> nodes) {
	...
	for (SVGCircle node : nodes)
		...
		if (node != parent
			&& ((SimpleNodeObject) node.getObject()).getParentID() == ((SimpleNodeObject) parent
				.getObject()).getNodeID()) {
	...
	SVGUtils.performLayout(nodes, edges, SVGUtils.TREE_HORIZ, svgPage
		.getWidth(), svgPage.getHeight());
...
class SimpleNodeObject {
    ...
    public SimpleNodeObject(int nodeID, int parentID, int requestType,
