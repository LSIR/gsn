...
public class VirtualSensorPool {
    ...
    private static final transient Logger logger = Logger
	    .getLogger(VirtualSensorPool.class);
	...
	if (logger.isInfoEnabled())
	    logger.info((new StringBuilder("Preparing the pool for: ")).append(
		    instance.getConfig().getVirtualSensorName()).append(
		    " with the max size of:").append(maxPoolSize).toString());
    ...
    public synchronized VirtualSensor borrowObject()
	    throws PoolIsFullException,
	...
	if (idleInstances.size() > 0)
	    ...
	    try {
		newInstance = (VirtualSensor) Class
			.forName(processingClassName).newInstance();
	...
	if (isDebugEnabled)
	    logger.debug(new StringBuilder().append("VSPool Of ").append(
		    config.getVirtualSensorName()).append(
		    " current busy instances : ").append(currentPoolSize)
		    .toString());
	return newInstance;
    }
	...
	for (VirtualSensor o : allInstances)
	    o.finalize(map);
	if (isDebugEnabled)
	    logger.debug(new StringBuilder().append("The VSPool Of ").append(
		    config.getVirtualSensorName()).append(" is now closed.")
		    .toString());
    }
}
