...
public class LoginToMSRSense {
	...
	public static int register_sensor(String username,String password,VSensorConfig conf,String gsnURI ) throws RemoteException, FileNotFoundException {
		
		gsn.msr.sensormap.sensorman.ServiceStub.Guid passGUID = login_to_sensor_map(username, password);
		HashMap<String, MetaData> output = MetaData.createMetaData(Main.getContainerConfig().getMsrMap().get("metadata"));
		...
		logger.info("Creating a group called : "+conf.getName()+ " With Parent: "+groupName+" MSR: "+call_output);
		...
		deleteVSensorParam.setSensorType("GSN-Proj-"+conf.getName());
		...
		registerVectorSensorParams.setVectorSensorType("GSN-Proj-"+conf.getName());
		registerVectorSensorParams.setPTypes(pTypes.toString());
		registerVectorSensorParams.setPComments(pComments.toString());
		registerVectorSensorParams.setPMetaData(pMetadata.toString());
