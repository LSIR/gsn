...
//		toReturn.setGetAggregateScalarDataResult(items);
	...
	class SignalRequest {
		...
		public SignalRequest(String req) {
			...
			if (signal_index>=conf.getProcessingClassConfig().getOutputFormat().length)
				throw new RuntimeException("Bad request: vs-name="+vsName+", "+signal_index+">"+conf.getProcessingClassConfig().getOutputFormat().length);
		}
		public String getFieldName() {
			return conf.getProcessingClassConfig().getOutputFormat()[signal_index].getName();
		}
