...
public final class StreamSource {
    ...
    * @param addressing
    */
   public StreamSource ( final String alias , final String query , final String size , final ArrayList < AddressBean > addressing ) {
      this.alias = alias;
      this.sqlQuery = query;
   ...
   public void setUsedDataSource ( final StreamProducer ds , final DataListener dbDataListener ) {
   ...
   public boolean validate ( ) {
      if ( this.storageHistorySize != null ) {
         this.storageHistorySize = this.storageHistorySize.replace( " " , "" ).trim( ).toLowerCase( );
         if ( this.storageHistorySize.equalsIgnoreCase( "" ) ) return true;
         ...
         if ( mIndex < 0 && hIndex < 0 && sIndex < 0 ) {
            try {
               this.parsedStorageSize = Integer.parseInt( this.storageHistorySize );
               this.isStorageCountBased = true;
            } catch ( final NumberFormatException e ) {
               logger.error( new StringBuilder( ).append( "The storage size, " ).append( this.storageHistorySize ).append( ", specified for the Stream Source : " ).append( this.getAlias( ) ).append(
            ...
            try {
               final StringBuilder shs = new StringBuilder( this.storageHistorySize );
