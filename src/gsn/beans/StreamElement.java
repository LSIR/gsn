...
public final class StreamElement implements Serializable {
    ...
    public StreamElement(final Collection<DataField> outputStructure,
	    final Serializable[] data, final long timeStamp) {
	this.fieldNames = new String[outputStructure.size()];
	this.fieldTypes = new Integer[outputStructure.size()];
	this.timeStamp = timeStamp;

	final DataField[] outputStructureArr = outputStructure
		.toArray(new DataField[] {});
    ...
    public StreamElement(final String[] dataFieldNames,
	    final Integer[] dataFieldTypes, final Serializable[] data,
    ...
    private void verifyTypesCompatibility(final Integer[] fieldTypes,
		...
		if (!(data[i] instanceof Short))
		    throw new IllegalArgumentException(
			    "The newly constructed Stream Element is not consistant. The "
				    + (i + 1)
				    + "th field is defined as "
				    + DataTypes.TYPE_NAMES[i]
	...
	for (int i = 0; i < this.fieldNames.length; i++)
	    output.append(",").append(this.fieldNames[i]).append("/").append(
    ...
    /**
         * This method gets the attribute name as the input and returns the
         * value corresponding to that tuple.
         * 
         * @param fieldName
	...
	if (this.indexedFieldNames == null) {
	    this.indexedFieldNames = new TreeMap<String, Integer>(
