...
public class DataTypes {
    ...
    // NEXT FIELD
    public final static String BINARY_PATTERN_STRING = "\\s*BINARY"
	    + OPTIONAL_NUMBER_PARAMETER + "(\\s*:\\s*\\w*)?\\s*";
	...
	for (int i = 0; i < DataTypes.ALL_PATTERNS.length; i++)
		...
		return i;
	throw new GSNRuntimeException(new StringBuilder("The type *").append(
		type).append("* is not recognized.").append(
		DataTypes.ERROR_MESSAGE).toString());
    }

    public static Integer convertFromJDBCToGSNFormat(
	    final int colTypeInJDBCFormat) {
	...
	case Types.LONGVARBINARY:
	    return BINARY;
	default:
	    logger.error("The type can't be converted to GSN form : "
		    + colTypeInJDBCFormat);
	    break;
	}
	return null;
