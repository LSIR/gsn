 ...
 */
public class VSensorLoader implements Runnable {
	...
	private long pidFileLastModified;
	...
	public VSensorLoader(String pluginsDir, File pidFile) {
		...
		this.pidFileLastModified = pidFile.lastModified();
		Thread thread = new Thread(this);
		thread.setName("VSensorLoader-Thread" + THREAD_COUNTER);
		thread.start();
		Thread directoryRefreshingThread = new Thread(
				new TheDirectoryRefreshing());
		directoryRefreshingThread.setName("Directory-RefereshingThread"
				+ THREAD_COUNTER++);
		directoryRefreshingThread.start();
	}
	...
	public void loadPlugin() throws SQLException, JiBXException {
		Modifications modifications = getUpdateStatus();
		...
		for (String configFile : removeIt) {
			...
			toDirectoryService(sensorInstance.getConfig(), Registry.DEREGISTER);
			Mappings.removeFilename(configFile);
			removeAllResources(sensorInstance);
		}
		if (pidFile.lastModified() != pidFileLastModified) {
			if (logger.isInfoEnabled())
				logger.info("Terminate Request Received");
			for (String configFile : Mappings.getAllKnownFileName()) {
				String name = Mappings.getConfigurationObject(configFile)
						.getVirtualSensorName();
				logger.warn(new StringBuilder().append("removing : ").append(
						name).toString());
				VSensorInstance sensorInstance = Mappings
						.getVSensorInstanceByFileName(configFile);
				toDirectoryService(sensorInstance.getConfig(),
						Registry.DEREGISTER);
				Mappings.removeFilename(configFile);
				removeAllResources(sensorInstance);
			}
		...
		configFilesLoop: for (String configFile : addIt) {
			...
			createInputStreams(configuration, sensorInstance.getPool());

			try {
				storageManager.createTable(sensorInstance.getConfig()
			...
			toDirectoryService(configuration, Registry.REGISTER);
		}
	}
	...
	private synchronized void toDirectoryService(VSensorConfig configuration,
		...
		if (postMethod == null) {
			...
			fillParameters(postMethod, configuration, action);
			directoryCommunicationCache.put(configuration, postMethod);
		}
		if (action == Registry.DEREGISTER) {
			postMethod.setRequestHeader(Registry.REQUEST, Integer
					.toString(Registry.DEREGISTER));
			directoryCommunicationCache.remove(configuration);
		}
	...
	class TheDirectoryRefreshing implements Runnable {
		public void run() {
			...
			while (true) {
				try {
					...
					while (keys.hasNext())
						toDirectoryService(keys.next(), Registry.REGISTER);
	...
	private void removeAllResources(VSensorInstance sensorInstance) {
		...
		Mappings.getContainer().removeAllResourcesAssociatedWithVSName(
				vsensorName);
		storageManager.dropTable(config.getVirtualSensorName());
	}

	public Modifications getUpdateStatus() {
		return this.getUpdateStatus(pluginsDir);
	}
	...
	private void prepareStreamSource(InputStream inputStream,
		...
		context.put(STORAGE_MANAGER, storageManager);
		for (Iterator<AddressBean> addresses = streamSource.getAddressing()
				.iterator(); addresses.hasNext();) {
			AddressBean addressBean = addresses.next();
			...
			if (ds == null) {
				if (!addressBean.isAbsoluteAddressSpecified()) {// Dynamic-address
					ArrayList<VirtualSensorIdentityBean> resolved = resolveByDirecotryService(addressBean
							.getPredicates());
					if (resolved.size() == 0) {
						logger.warn(new StringBuilder().append(
					 ...
					 * returned from directory service and ignoring the rest.
					 */
