...
public class FieldDownloadServlet extends HttpServlet {
	...
	if (type == null)
	    ...
	    type = type.trim();
	StringBuilder query = new StringBuilder().append(prefix)
		.append(vs_name).append(postfix);
	byte[] picture = StorageManager.getInstance().getBinaryFieldByQuery(
		query, colName, Long.parseLong(primaryKey));
	if (picture == null)
