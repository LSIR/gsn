 ...
 *         FIXME : 1. Because a prepared statements relies on the connection
 *         being there. If a connection times out the pool will restore it and
 *         then the prepared statement is then stale. The only way you can tell
 *         this it to use it, and any speed advantage is negated when you have
 *         to try it, catch the error, regen the statement and go again.
 * 
 * 2. You use a connection pool in threaded enviroment. A prepared statement is
 * a single instance that can only be used in a single thread. So either you
 * block all the threads waiting for the single prepared statement (and then you
 * have to wonder why you are using threads) or you have multiple prepared
 * statements for same thing.
 * 
 */
public class DataEnumerator implements Enumeration {
	...
	for (int i = 1; i <= resultSet.getMetaData().getColumnCount(); i++) {
		...
		fieldTypes.add(DataTypes
			.convertFromJDBCToGSNFormat(colTypeInJDBCFormat));
	    }
	}
	...
	try {
	    ...
	    for (int i = 1, colIndex = 0; i <= resultSet.getMetaData()
		    .getColumnCount(); i++) {
		...
		streamElement = new StreamElement(dataFieldNames,
			dataFieldTypes, output, resultSet
				.getLong(StreamProducer.TIME_FIELD));
	    else
		streamElement = new StreamElement(dataFieldNames,
			dataFieldTypes, output, System.currentTimeMillis());
	    hasNext = resultSet.next();
	    if (hasNext == false)
