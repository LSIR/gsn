...
public class StorageManager {
    ...
    public void createTable(String tableName, Collection<DataField> structure, boolean storageEnabled, boolean overwrite) throws SQLException {
        ...
        if (overwrite)
            ...
            try {
                ...
                if (exists == true) {
                    ...
                    if (isMysqlDB())
                        rsmd = connection.createStatement().executeQuery(new StringBuilder("select * from ")
                                .append(tableName).append(" limit 0").toString()).getMetaData();
                    if (isHsql()) {
                    ...
                    for (DataField df : structure) {
                        for (int i = 0; i < rsmd.getColumnCount(); i ++)
                            if (rsmd.getColumnName(i).equals(df.getFieldName()))
                                if (DataTypes.convertFromJDBCToGSNFormat(rsmd.getColumnType(i)) == df.getDataTypeID())
                                    continue;
                                else {
                                    logger.error("The table >" + tableName + "< exists but the structure is not compatible with what GSN desires.");
                                    System.exit(1);
                                }

                        logger.error("The field >" + df.getFieldName() + "< is missing from table >" + tableName + "<.");
                        System.exit(1);
                    }
                }
        ...
        String sqlCreateStatement = getCreateTableStatement(structure, tableName, storageEnabled).replace("\"", "");
        String sqlCreateIndexStatement = new StringBuilder("CREATE INDEX ")
                .append(tableName).append("_INDEX ON ").append(tableName).append(" (TIMED DESC)").toString();
        if (isDebugEnabled == true)
            logger.debug(new StringBuilder().append("The create table statement is : ").append(sqlCreateStatement).toString());
        if (isDebugEnabled == true)
            logger.debug(new StringBuilder().append("The create index statement is : ").append(sqlCreateIndexStatement).toString());
     ...
     */
    protected String getCreateTableStatement(Collection<DataField> structure, String alias, boolean permanantStorageEnabled) {
            ...
            if (StorageManager.isMysqlDB()) {
                switch (field.getDataTypeID()) {
                    ...
                    case DataTypes.VARCHAR :
                        // Because the parameter for the varchar is not optional.
     ...
     * @param duplicateErrorSuspend, If set to true, the method ignores the exception caused by the duplicated rows in the specified table.
    ...
    public boolean insertData(String tableName, StreamElement se, boolean duplicateErrorSuspend)  {
        ...
        try {
            ...
            if (duplicateErrorSuspend && e.getMessage().contains("Unique constraint violation"))
                return true;
            ...
            if (e.getType() == GSNRuntimeException.UNEXPECTED_VIRTUAL_SENSOR_REMOVAL) {
                if (isDebugEnabled == true)
                    logger.debug("An stream element dropped due to unexpected virtual sensor removal.", e);
            } else
     ...
     * @param tableName The database name which should be already created.
     * @param se        The stream element to Insert in to the specified database.
     *                  This method has the <code>duplicateErrorSuspend<code> set to false hence the method generates exceptions in the case of attempting to generate
     ...
     *                  This method has the <code>duplicateErrorSuspend<code> set to true hence the method won't generate any exceptions caused by attempting to
    ...
    /**
     * Creates a sql statement which can be used for inserting the specified stream element in to the specified table.
     *
     ...
     * @return A sql statement which can be used for inserting the provided stream element into the specified table.
     */
    private StringBuilder getInsertStatement(String tableName, StreamElement se) {
        ...
        for (String fieldName : se.getFieldNames()) {
            if (fieldsDefinedToBeStored == null) {
                throw new GSNRuntimeException("Unexpected virtual sensor removal", GSNRuntimeException.UNEXPECTED_VIRTUAL_SENSOR_REMOVAL);
            }
        ...
        for (int i = 0; i < se.getFieldNames().length; i ++) {
            if (fieldsDefinedToBeStored.contains(se.getFieldNames()[i].toUpperCase()))
                toReturn.append(" ? ,");
        }
        toReturn.append(" ? )");
        return toReturn;
    ...
    /**
     * Drops the specified table. Note that the table is going to be droped only
     * when the parameter <code>overwrite-tables</code> in the container's main
     * configuration file is set to true.
     *
     * @param tableName The name of the table to be drop.
     */
     ...
     *                       Overwrite the parameter <code>overwrite-tables</code> in the
    ...
    public void dropTable(String tableName, boolean forceDropTable) {
        ...
        if (StorageManager.getInstance().isHsql())
            dropIndexStatement = new StringBuilder(" DROP INDEX ").append(tableName.toUpperCase()).append("_INDEX IF EXISTS ").toString();
    ...
    public void initialize(String databaseDriver, String databaseUserName, String databasePassword, String databaseURL) {
        if (databaseDriver.trim().equalsIgnoreCase("org.hsqldb.jdbcDriver"))
            ...
            logger.error(new StringBuilder().append("Please check the storage element in the file : ").append(getContainerConfig()
                    .getContainerFileName()).toString());
            System.exit(1);
        }
    ...
    public FastConnectionPool(String driverClassName, String username, String password, String databaseURL, int maximumOpenConnectionsToDB) {
        ...
        try {
            ...
            logger.error(new StringBuilder()
                    .append("Connecting to the database with the following properties failed :").append("\n\t UserName :").append(username)
