...
public class SQLUtils {
    ...
    /**
         ...
         * @param sql
         * @param mapping
         *                (The keys should be in UPPER CASE)
         */
    ...
    public static ArrayList<String> extractTableNamesUsedInQuery(
	    StringBuilder query) {
	...
	int endIndex;
	if (indexOfWhere > 0)
	    endIndex = indexOfWhere;
	else if (indexOfGroupBy > 0)
	    endIndex = indexOfGroupBy;
	else if (indexOfOrderBy > 0)
	    endIndex = indexOfOrderBy;
	else
	...
	if (input.length() > 0)
	    result.add(input.toString().trim());
	return result;
