...
public class PIDUtils {
   ...
   private static transient Logger logger                = Logger.getLogger( PIDUtils.class );
   
   public static final String      GSN_PID               = "gsn.pid";
   
   public static File createPID ( String pid_file ) throws IOException , RuntimeException {
      String tempDirPath = System.getProperty( "java.io.tmpdir" );
      File tempFile = new File( tempDirPath + "/" + pid_file );
      if ( tempFile.exists( ) ) throw new RuntimeException( "The " + tempFile.getAbsolutePath( ) + " file exists. Start failed." );
   ...
   public static void killPID ( File tempFile ) throws IOException {
      if ( !tempFile.isFile( ) || !tempFile.exists( ) ) {
         logger.warn( "Killing failed : " + "The " + tempFile.getAbsolutePath( ) + " file doesn't exists. Stop failed." );
         return;
      }
   ...
   public static int getFirstIntFrom ( File file ) throws IOException {
      ...
      int toReturn = -1;
      if ( fis.available( ) > 0 ) toReturn = fis.read( );
      fis.close( );
      return toReturn;
   }
}
