...
//		wrapper.initialize(fields,formats);
	...
	public void testCheckpoints() throws IOException {
		...
		assertEquals(wrapper.run(new StringReader(data), CHECK_POINT_DIR).size(), parsed.size());
		...
		assertEquals(0,wrapper.run(new StringReader(data), CHECK_POINT_DIR).size());
		...
		assertEquals(1,wrapper.run(new StringReader(data), CHECK_POINT_DIR).size());
		data="###########################\n\n\n\n,,,,,,,,,\n\n\n"; // Empty File.
