...
class SensorStore(ds:DataStore) extends Actor{
  ...
  val confWatch: ActorRef =context.actorOf(Props[ConfWatcher], "ConfWatcher")
...
//  val connection:MongoConnection = null//driver.connection(List("localhost"))
  ...
  implicit val exe: ExecutionContextExecutor =context.dispatcher
  val refresh: Cancellable = context.system.scheduler.schedule(0 millis, 10 minutes, self, RefreshStats)
  object RefreshStats
  ...
  def extract_latest(series:Seq[Series], n:String): Option[Double] =
    series.find { x => x.output.fieldName.equalsIgnoreCase(n) }.flatMap { x => x.series.headOption }
  ...
  def receive: Receive ={
    ...
    case ModifiedVsConf(vs)=>
      ...
      implicit val source: Option[String] =vsDatasources.get(vsname)
    ...
    case RefreshStats=>
      ...
      sensors.foreach{case (sensorid,s)=>
        ...
        implicit val source: Option[String] =vsDatasources.get(sensorid)
        val stat = stats(s)
...
//    val db = connection.database("gsn-metadata")
